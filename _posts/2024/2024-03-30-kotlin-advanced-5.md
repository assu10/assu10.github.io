...
예를 들어 라이브러리가 특별한 함수 안에서 초기화를 해야 한다고 요구할 수도 있다.  
...
이 라이브러리는 _Bag_ 의 생성자에서 프로퍼티를 초기화하지 않기 때문에 파생 클래스가 반드시 _setUp()_ 안에서 초기화를 해야 한다고 요구한다고 가정해보자.
...
**_items_ 를 null 이 될 수 있는 String? 으로 선언하면 _checkItems()_ 처럼 모든 멤버 함수에서 null 검사**를 해야 한다.  
...
이럴 경우 **`.isInitialized` 를 사용하면 `lateinit` 프로퍼티가 초기화되었는지 판단**할 수 있다.
