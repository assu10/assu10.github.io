...
엔지니어가 기술 그 자체에 몰두하는 이유는 **현재 주어진 비즈니스 문제에 대한 도전 욕구가 충족되지 않기 때문**일 수 있다.  
...
비즈니스 로직이 복잡한 알고리즘 또는 복잡한 프로세스 규칙을 가진다면 일반적으로 핵심 하위 도메인이다.  
...
이러한 유스케이스 집합에서는 보통 동일한 행위자(actor), 비즈니스 엔티티를 포함하고 모두 밀접하게 관련된 데이터의 집합을 다룬다.
...
BusVNext 는 대중교통 회사로 고객에게 택시를 잡는 것처럼 쉽게 버스를 타는 경험을 제공하는 것이 목표이다.  
...
BusVNext 의 비즈니스 도메인은 대중교통이며, 고객에게 최적화된 버스 승차 서비스를 제공한다.
...
시스템 분석가와 엔지니어는 비즈니스 도메인의 멘탈 모델은 소프트웨어 요구사항과 소스 코드로 구현한다.
