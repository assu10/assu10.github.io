...
> 브로커의 `broker.rack` 설정 역시 원래는 레플리카들이 서로 다른 서버 rack 에 배치되도록 함으로써 한꺼번에 작동 불능에 빠지는 것을 방지하기 위해 고안된 것임  
> 하지만 시간이 흐르면서 클라우드 환경에서 카프카를 작동시키는 것이 더 일반적이 되었고, `broker.rack` 설정 역시 물리적 서버 rack 이라기 보다 클라우드의 
> 가용 영역을 가리키는 경우가 많아짐
> 
