...
* [1. 연산자 오버로딩: `operator`](#1-연산자-오버로딩-operator)
  ...
  * [1.3. 산술 연산자](#13-산술-연산자)
    ...
    * [1.3.6. 반환 타입이 두 피연산자의 타입과 다른 경우](#136-반환-파입이-두-피연산자의-타입과-다른-경우)
...
`equals()` 를 호출해야 하는 자바와 달리 코틀린은 `==` 비교 연산자를 직접 사용할 수 있어서 비교 코드가 자바에 비해 더 간결하다.
...
**`equals()` 는 확장 함수로 정의할 수 없는 유일한 연산자**이다.
...
자바에서는 primitive 타입에 대해서만 산술 연산자를 사용할 수 있고, 추가로 String 에 대해 `+` 연산자를 사용할 수 있다.
...
물론 변수가 변경 가능한 var 일 때만 복합 대입 연산자 사용이 가능하다.
...
> **교환 법칙 (commutativity)**
> 
> _a op b == b op a_ 인 성질

따라서 위의 코드에서 _p * 1.5_ 를 _1.5 * p_ 로도 사용해야 한다면 아래처럼 이에 대응하는 연산자 함수를 정의해야 한다.
...
### 1.3.6. 반환 타입이 두 피연산자의 타입과 다른 경우
