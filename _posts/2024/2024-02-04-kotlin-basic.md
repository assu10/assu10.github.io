...
* [1. 코틀린](#1-코틀린)
  * [1.1. 코틀린 특성](#11-코틀린-특성)
    ...
    * [1.1.2. 함수형 프로그래밍](#112-함수형-프로그래밍)
      * [1.1.2.1. 함수형 동작: 예측 가능한 도메인 로직을 위한 순수 함수 설계](#1121-함수형-동작-예측-가능한-도메인-로직을-위한-순수-함수-설계)
...
즉, **복잡한 객체의 상태 변경 대신, 불변 객체를 생성하여 새로운 결과를 반환**한다.

<**함수형 동작의 장점**>
- **예측 가능성**
  - 동일한 입력 → 항상 동일한 출력
- **테스트 용이성**
  - 상태나 환경 의존성이 없기 때문에 단위 테스트에 유리
- **사이드 이펙트 없음**
  - 외부 시스템, 전역 상태를 건드리지 않음
- **조합 가능성**
  - 출력이 다른 함수의 입력으로 바로 사용 가능(함수 합성)

<br />

<**함수형 동작이 필요한 이유**>  
도메인 서비스나 애그리거트 내부에서 **복잡한 규칙 계산**을 수행할 때, 객체의 내부 상태를 변경하기보다는 **부수 효과 없는 함수형 접근법**을 적용하는 것이 
유지보수와 품질 관점에서 더 안전하다.
- **도메인 개념을 불변 구조로 정의**하고,
- **구체적인 동작은 순수 함수로 구현**

이 두 단계를 통해 **예측 가능하고 테스트 가능한 도메인 로직**을 만들 수 있다.

전통적인 OOP 방식에서는 내부 상태를 직접 변경하지만, **함수형 접근법에서는 상태를 변경하지 않고 새로운 객체를 반환**한다.
...
fun recordQuoteLine(
    ...
    quoteLine: QuoteLine
): Pair<PolicyQuote, List<DomainEvent>> {
    val newPolicyQuote = policyQuote.withNewQuoteLine(quoteLine)
    val event = QuoteLineRecorded.of(policyQuote.id, quoteLine)

    return Pair(newPolicyQuote, listOf(event))
}
```

- _policyQuote_ 는 불변이다.
- _recordQuoteLine()_ 은 입력을 변경하지 않으며, **새로운 상태 + 도메인 이벤트를 반환**한다.
- 이는 명확한 동작 결과를 보장하며, 부수 효과가 없다.
- 이 패턴은 특히 이벤트 소싱 또는 CQRS 아키텍처와 결합할 때 더욱 강력하다.

<**함수형 코어와 부수 효과의 분리**>
- **도메인 로직은 순수 함수 기반으로 설계**하고,
- **부수 효과(IO, DB 등)는 별도로 분리**하면,
- 시스템 안정성과 테스트 커버리지가 크게 향상된다.

복잡성을 피하는 유일한 방법은 **코드 단순화**이며, 함수형 동작은 이를 위한 가장 효과적인 도구 중 하나이다.
...
- **객체 지향 프로그래밍(OOP, Object-Oriented Programming)**
  ...
  - 예) 계좌 객체는 잔액이라는 상태를 가지고 있고, 입금과 출금 메서드를 통해 상태를 변경함
...
- **함수형 프로그래밍(FP, Functional Programming)**
  ...
  - 고차 함수: 함수를 매개 변수로 받거나 반환값으로 사용하는 함수
...
어떤 객체를 다른 타입으로 캐스트하기 전에 타입 검사를 하지 않으면 `ClassCastException` 이 발생할 수도 있는데 자바에서는 타입 검사와 타입 캐스트를 각각 해야 한다.    
...
예) Int 타입을 컬렉션의 타입 파라미터로 넘기면 그 컬렉션에는 Int 의 래퍼 타입에 해당하는 java.lang.Integer 객체가 들어감
