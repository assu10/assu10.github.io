---
layout: post
title: "Kotlin - 확장 람다, StringBuilder, buildString(), 영역 함수"
date: 2024-03-16
categories: dev
tags: kotlin filterIndexed() toCharArray() stringBuilder buildString() buildList() buildMap() forEachIndexed() let() run() with() apply() also() removeSuffix() takeUnless()
---
...
<!-- TOC -->
* [1. 확장 람다](#1-확장-람다)
  * [1.1. 여러 파라미터를 받는 확장 람다](#11-여러-파라미터를-받는-확장-람다)
  * [1.2. 함수의 파라미터로 확장 람다 사용](#12-함수의-파라미터로-확장-람다-사용)
  * [1.3. 확장 람다의 반환 타입이 Unit 인 경우](#13-확장-람다의-반환-타입이-unit-인-경우)
  * [1.4. 일반 람다를 파라미터로 받는 위치에 확장 람다 전달: `filterIndexed()`, `toCharArray()`](#14-일반-람다를-파라미터로-받는-위치에-확장-람다-전달-filterindexed-tochararray)
...
// 확장 람다
// String 파라미터를 괄호 밖으로 옮겨서 String.(Int) 처럼 확장 함수 구문 사용
...
아래 확장 함수 구문을 보자.  
다른 람다와 마찬가지로 파라미터가 하나(여기서는 Int) 뿐이면 확장 함수에서도 `it` 으로 그 유일한 파라미터를 가리킬 수 있다.
```kotlin
// String 파라미터를 괄호 밖으로 옮겨서 String.(Int) 처럼 확장 함수 구문 사용
// 확장 대상 객체(여기서는 String) 이 수신 객체가 되고, this 를 통해서 수신 객체에 접근 가능
val vb: String.(Int) -> String = {
...
확장 람다인 위 함수에서 String 을 확장하는 건 파라미터 목록인 (Int) 가 아니라 전체 람다인 _(Int) -> String_ 이다.  
따라서 확장 람다는 _String.(Int) -> String_ 에서 _(Int) -> String_ 이 부분이다. 

코틀린에서는 **확장 람다를 수신 객체가 지정된 함수 리터럴** 이라고 한다.  
**함수 리터럴은 람다와 익명 함수 모두를 포함**한다.  
따라서 수신 객체가 암시적 파라미터로 추가 지정된 람다라는 사실을 강조하고 싶을 때는 **수신 객체가 지정된 람다**를 **확장 람다**의 동의어로 더 자주 사용한다.

---

## 1.1. 여러 파라미터를 받는 확장 람다

확장 함수처럼 확장 람다도 여러 파라미터를 받을 수 있다.

```kotlin
// Int 의 확장 람다
// 파라미터는 아무것도 받지 않고 Boolean 리턴 () -> Boolean
val zero: Int.() -> Boolean = {
    this == 0
}

// 파라미터에 이름을 붙이는 대신 `it` 사용
...
## 1.2. 함수의 파라미터로 확장 람다 사용

위에선 val 를 선언하여 확장 람다를 이용했지만 아래의 _f2()_ 처럼 **함수의 파라미터로 확장 람다를 사용하는 것이 일반적**이다.
...
## 1.4. 일반 람다를 파라미터로 받는 위치에 확장 람다 전달: `filterIndexed()`, `toCharArray()`

**일반 람다를 파라미터로 받는 위치에 확장 람다를 전달**할 수도 있는데 **이 때 두 람다의 파라미터 목록을 서로 호환**되어야 한다.

```kotlin
// 일반 람다를 파라미터로 받는 String 의 확장 함수
fun String.transform1(
    n: Int,
    lambda: (String, Int) -> String,
) = lambda(this, n)

// 확장 람다를 파라미터로 받는 String 의 확장 함수
...
확장 함수에 대한 참조는 확장 람다의 타입과 같다.

위 예시에서 _Int::f2_ 는 _Int.() -> Int_ 이다.

_11.d1(Int::f2)_ 호출을 보면 일반 람다 파라미터를 요구하는 _d1()_ 에 확장 함수를 넘기고 있다.
...
위에서 `with()` 는 파라미터가 2개 있는 함수이다.  
첫 번째 파라미터는 _result_ 이고, 두 번째 파라미터는 람다이다.    
...
만일 안전한 호출을 하지 않는다면 영역 함수 안에서 개별적으로 null 검사를 해야 한다.
