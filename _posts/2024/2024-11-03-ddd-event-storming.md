...
- 이벤트는 커맨드 없이도 발생할 수 있다. 예를 들어 시간 기반 스케줄링은 커맨드없이 이벤트를 유발한다.
...
참가자들이 이벤트, 커맨드, 액터 등 모델링 요소에 부여된 색을 기억하기 쉽게 포스트잇을 사용하여 워크숍이 진행되는 동안 모든 참가자들이 볼 수 있게 붙여둔다.
...
더 많은 참가자가 지식 공유를 해야 한다면 세션을 나누어서 진행한 후 각각의 결과를 비교해서 취합된 모델을 만든다.
