...
독립적인 함수를 정의할 수 있으므로 모든 것을 클래스 안에 가둘 필요 없이 확장 함수를 사용하여 상속을 쓰지 않아도 기능 확장이 가능하다.
...
업캐스트가 이루어지면서 각 객체가 _Circle_, _Square_, _Triangle_ 중 어느 타입인지에 대한 구체적인 정보가 사라지고, 모두 그냥 _Shape_ 객체로 취급된다.
...
**상속을 하면서 함수를 추가하는 것은 클래스에 기반 클래스가 있다는 사실을 무시하고 시스템 전반에서 파생 클래스를 엄격하게 식별하여 취급할 때 유용**하다.  
...
하지만 이런 방식은 상속을 하면서 클래스를 확장하긴 하지만 새 멤버 함수는 오직 라이브러리에 연결하기 위해서만 사용된다.  
...
위 코드를 보면 **멤버 함수인 _f()_ 에서는 다형성이 동작하지만, 확장 함수인 _g()_ 에서는 작동하지 않는 것**을 알 수 있다.

함수를 오버라이드할 필요가 없고, 클래스의 공개 멤버만으로 충분할 때는 이를 멤버 함수로 구현하든 확장 함수로 구현하든 상관없다.
