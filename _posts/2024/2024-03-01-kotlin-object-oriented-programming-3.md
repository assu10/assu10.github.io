...
* [1. 클래스 위임 (class delegation)](#1-클래스-위임-class-delegation)
  ...
  * [1.4. 상속의 제약과 합성 → 상속 → 클래스 위임](#14-상속의-제약과-합성--상속--클래스-위임)
...
> 합성에 대한 내용은 [4. 합성 (Composition)](https://assu10.github.io/dev/2024/02/25/kotlin-object-oriented-programming-2/#4-%ED%95%A9%EC%84%B1-composition) 을 참고하세요.
...
이제 `by` 를 사용하여 [1.1. 수동으로 위임](#11-수동으로-위임) 의 _ExplicitControls_ 을 재작성해본다.
...
<**스마트 캐스트가 되지 않는 경우**>  
- 특정 식이 재계산 될 수 있는 경우
- 상속을 위해 `open` 된 프로퍼티도 파생 클래스에서 오버라이드를 할 수 있고, 그로 인해 프로퍼티에 접근할 때마다 항상 같은 값을 내놓는다고 보장할 수 없으므로 스마트 캐스트가 안됨
...
// 안전한 캐스트 `as?` 로 캐스트 수행 후 엘비스 연산자 `?:` 사용
...
아래는 안전한 연산자 `as?` 를 사용하여 `equals()` 메서드를 구현한 예시이다.
...
`sealedSubclasses` 는 다형적인 시스템을 만들 때 중요한 도구가 될 수 있는데, 새로운 클래스가 모든 적합한 연산에 자동으로 포함되도록 보장할 수 있다.

하지만 **`sealedSubclasses` 는 파생 클래스를 실행 시점에 찾아내므로 시스템 성능에 영향**을 미칠 수 있기 때문에, 만일 성능 문제가 발생한다면 프로파일러를 사용하여
