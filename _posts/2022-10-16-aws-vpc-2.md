...
>   - Resource 삭제
> - Endpoint Service 로 Private Link 구성 테스트
>   - 기본 환경 구성
...
> `Interface Endpoint` 와 `Endpoint Service` 를 통한 연결을 `Private Link` 라고 함.  
> 뒤의 *3. Endpoint Service 로 Private Link 구성 테스트* 를 참고하세요.
...
# S3 의 DNS 주소로 접근 시도
[ec2-user@ip-10-10-0-45 ~] ping s3.ap-northeast-2.amazonaws.com

PING s3.ap-northeast-2.amazonaws.com (52.219.144.81) 56(84) bytes of data.
64 bytes from s3.ap-northeast-2.amazonaws.com (52.219.144.81): icmp_seq=1 ttl=57 time=0.235 ms
64 bytes from s3.ap-northeast-2.amazonaws.com (52.219.144.81): icmp_seq=2 ttl=57 time=0.280 ms
```
...
[ec2-user@ip-10-10-0-45 ~]$ ssh root@10.10.1.28  # Private EC2 로 접속

[root@ip-10-10-1-28 ~] ping s3.ap-northeast-2.amazonaws.com
PING s3.ap-northeast-2.amazonaws.com (52.219.146.0) 56(84) bytes of data.
```
...
PING s3.ap-northeast-2.amazonaws.com (52.219.148.36) 56(84) bytes of data.
64 bytes from s3.ap-northeast-2.amazonaws.com (52.219.148.36): icmp_seq=1 ttl=57 time=0.192 ms
64 bytes from s3.ap-northeast-2.amazonaws.com (52.219.148.36): icmp_seq=2 ttl=57 time=0.213 ms
```
...
[ec2-user@ip-10-10-0-45 ~] dig +short cloudformation.ap-northeast-2.amazonaws.com
52.95.193.155

[ec2-user@ip-10-10-0-45 ~]$ ping cloudformation.ap-northeast-2.amazonaws.com
PING cloudformation.ap-northeast-2.amazonaws.com (52.95.193.132) 56(84) bytes of data.
64 bytes from 52.95.193.132 (52.95.193.132): icmp_seq=1 ttl=248 time=0.228 ms
64 bytes from 52.95.193.132 (52.95.193.132): icmp_seq=2 ttl=248 time=0.293 ms
```
...
[root@ip-10-10-1-28 ~]# dig +short cloudformation.ap-northeast-2.amazonaws.com
52.95.193.155
[root@ip-10-10-1-28 ~]# ping cloudformation.ap-northeast-2.amazonaws.com
PING cloudformation.ap-northeast-2.amazonaws.com (52.95.193.155) 56(84) bytes of data.
```
...
# Endpoint 전용 호스트
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-00c910bcce10a071d-m8shdg7j.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.0.150
10.10.1.156
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-00c910bcce10a071d-m8shdg7j-ap-northeast-2a.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.0.150
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-00c910bcce10a071d-m8shdg7j-ap-northeast-2c.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.1.156
```
...
# Endpoint 전용 호스트
[root@ip-10-10-1-28 ~]# dig +short vpce-00c910bcce10a071d-m8shdg7j.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.1.156
10.10.0.150
[root@ip-10-10-1-28 ~]# dig +short vpce-00c910bcce10a071d-m8shdg7j-ap-northeast-2a.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.0.150
[root@ip-10-10-1-28 ~]# dig +short vpce-00c910bcce10a071d-m8shdg7j-ap-northeast-2c.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.1.156
```

기본 환경과는 다르게 CloudFormation 서비스의 DNS 주소가 10.10.0.150, 10.10.1.156 Private IP 대역인 것을 확인할 수 있다.  
...
# Endpoint 전용 호스트
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-0f9f5cd0599e97634-nofr99pf.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.0.207
10.10.1.180
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-0f9f5cd0599e97634-nofr99pf-ap-northeast-2a.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.0.207
[ec2-user@ip-10-10-0-45 ~]$ dig +short vpce-0f9f5cd0599e97634-nofr99pf-ap-northeast-2c.cloudformation.ap-northeast-2.vpce.amazonaws.com
10.10.1.180
```
...
CloudFormation Stack 이 삭제되면 위의 *2. Gateway Endpoint 와 Interface Endpoint 비교 테스트 > CloudFormation 를 통해 생성된 자원* 의 자원이
모두 삭제되었는지 확인한다.

---

# 3. Endpoint Service 로 Private Link 구성 테스트
...
- Endpoint Service 생성/연결
  - Endpoint Service 생성
  - Interface Endpoint 생성/연결 (Private Link 생성)
  - Endpoint Service 검증
- Resource 삭제
...
[ec2-user@ip-10-10-0-201 ~]$ dig +short jh-CF-jhCus-ULNX38G0076B-1d22a84a855222d6.elb.ap-northeast-2.amazonaws.com
3.37.50.246

[ec2-user@ip-10-10-0-201 ~]$ curl jh-CF-jhCus-ULNX38G0076B-1d22a84a855222d6.elb.ap-northeast-2.amazonaws.com
...
만일 보안상 외부 인터넷 구간이 아닌 Private 네트워크를 통해 VPC 간 통신을 해야한다면 `Endpoint Service` 를 이용하여 `Private Link` 구성 후
통신이 가능하다.
...
Endpoint Service 를 생성하여 VPC 간 Private Link 연결 구성을 한다.

- Endpoint Service 생성
- Interface Endpoint 생성/연결 (Private Link 생성)
- Endpoint Service 검증

![Private Link 환경 구성도](/assets/img/dev/2022/1016/endpoint-service-10.png)
...
이제 Interface Endpoint 와 Private Link 를 연결할 수 있다.
...
```shell
$ ssh -i sandbox-jh.pem ec2-user@3.38.xx.xx
[ec2-user@ip-10-10-0-201 ~]$ dig +short vpce-0fdf2....ap-northeast-2.vpce.amazonaws.com
10.10.0.123
[ec2-user@ip-10-10-0-201 ~]$ curl vpce-0fdf2....ap-northeast-2.vpce.amazonaws.com
<html><h1>Endpoint Service Lab - CloudNeta Web Server 1</h1></html>
```
...
![VPC Endpoint 를 통한 Private Link 통신 흐름](/assets/img/dev/2022/1016/endpoint-service-10.png)

① *jhVPC* 내의 EC2 Instance 에서 *jhCustomVPC* 의 웹서버로 서비스 요청 시 Interface Endpoint 로 데이터 전달  
② Private Link 를 통해 Endpoint service 가 생성된 *jhCustomVPC* 내의 NLB 로 전달  
...
CloudFormation Stack 이 삭제되면 위의 *3. Endpoint Service 로 Private Link 구성 테스트 > CloudFormation 를 통해 생성된 자원* 의 자원이
모두 삭제되었는지 확인한다.
