...
<!-- TOC -->
  ...
  * [6. 컨피그 저장소의 중요 정보 보호(암호화)](#6-컨피그-저장소의-중요-정보-보호암호화)
...
새로운 스프링부트 프로젝트 생성 후 Config Server Dependency 를 추가한다.
...
스프링부트의 버전은 2.3.2 이고, 스프링 클라우드의 버전은 Hoxton.SR6 이다.
...
`@EnableConfigServer` 은 서비스를 컨피그 서버 서비스로 사용 가능하게 한다.
...
환경설정 정보가 변경될 때마다 인스턴스의 수만큼 종단점을 호출해줘야 한다.
