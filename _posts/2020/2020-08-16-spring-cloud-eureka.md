...
서비스 호출이 실패하면 로컬에 있던 캐시는 무효화되고 레지스트리로부터 새로 정보를 받아온다.
...
새로운 스프링부트 프로젝트 생성 후 Config Client, Eureka Server, Actuator Dependency 를 추가한다.
...
컨피그 서버 원격 저장소 각 환경설정 파일에 아래 구성 내용을 추가한다.
...
**eurekaserver > application-peer1.yaml**
```yaml
spring:
  application:
...
**eurekaserver > application-peer2.yaml**
```yaml
spring:
  application:
