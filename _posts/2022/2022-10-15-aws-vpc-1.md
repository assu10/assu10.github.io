...
현재 설정된 Public Routing 테이블에 모든 네트워크(0.0.0.0/0)(=외부 인터넷 통신을 위해) 가 IGW 로 향하는 라우팅 경로를 추가한다.
...
현재 설정된 Public Routing 테이블에 모든 네트워크(0.0.0.0/0)(=외부 인터넷 통신을 위해) 가 NAT G/W 로 향하는 라우팅 경로를 추가한다.
