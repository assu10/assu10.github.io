...
Master 서버에 장애가 발생하는 경우 Slave 서버는 Master 서버로 자동 전환(`FailOver`) 되지 않기 때문에 일시적인 서비스 중단이 발생한다.
...
> **Sentinel 서버 리더 선출**    
> `sentinel.conf` 파일의 `down-after-milliseconds` 는 주관적 다운 상태를 인지할 수 있도록 설정된 타임아웃 값인데 이 값을 각각 다르게 설정하여
