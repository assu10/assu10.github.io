...
권한 탈취의 우려가 있지만, 'key pair file' 방식은 개인키가 노출되지 않기 때문에 안전하고 key file로 로그인이 가능하여 암호를
입력하지 않아도 되어 편리하다.
...
마지막 로그인: 10월 17 04:30:45 UTC 2022 일시 pts/0
...
이후 스택을 삭제하면 자원을 일일히 수동 삭제할 필요 없이 자동으로 생성된 자원이 삭제된다.
