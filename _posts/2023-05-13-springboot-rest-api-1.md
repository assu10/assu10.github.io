...
> 모든 요청과 응답에 대해 공통 기느응ㄹ 일괄적으로 쉽게 추가 가능하기 때문에 인증, 인가도 쉽게 구현 가능  
...
서블릿 스택에서 WAS 는 스레드를 효율적으로 관리하기 위해 스레드 풀 포함한다. 스레드 풀은 WAS 가 기동될 때 초기화되고 종료될 때 정리된다. 사용자 요청마다 스레드를 생셩하기엔 생성 비용이 비싸므로
