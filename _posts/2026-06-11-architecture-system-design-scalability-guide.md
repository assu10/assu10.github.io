...
| 대표 기술  | NySQL, Oracle, PostgreSQL, MariaDB   | Redis, MongoDB, Cassandra  |
...
애플리케이션은 읽기 요청을 처리할 때 DB로 가기 전 먼저 캐시를 확인하는 캐시 우선 읽기 전략(Lock-Aside / Cache-Aside)을 주로 사용한다.
...
- **LFU(Least Frequently Used):** 지금가지 사용된 **빈도가 가장 낮은 데이터**를 우선 삭제하는 정책
...
API 요청이나 가변적 HTML 페이지 가은 **동적 콘텐츠 전달 속도**도 획기적으로 개선한다.

- **연결 최적화(Persistent Connection)**
  - 네트워크 통신에서 가장 오랜 시간이 걸리는 작업 중 하나는 TCP Handshake와 TLS/SSL 암호화 연결을 맺는 과정이다.
  - CloudFront는 전 세계 사용자 근처의 에지 로케이션과 AWS 내부 원본 서거 간의 전용 네트워크 연결을 항상 '최신 유지(Keep Alive)' 상태로 맺어준다.
...
다중 데이터 센터 아키텍처가 주는 고가용성을 완벽하게 우리려면 반드시 해결해야 하는 핵심 난제 2가지가 있다.
...
- **데이터 동기화**
  ...
  동부에서 작성했던 데이터가 존재하지 않을 수 있다. 이 문제를 박기 위한 보편적인 전략은 데이터를 여러 데이터 센터에 걸쳐 실시간으로 다중화하는 것이다.
...
- **왜 데이터가 누락되는 현상이 발생할까?**
  ...
  실시간으로 서부 센터 DB에 동기화되기 못하고 몇 초간의 지연(Lag)이 생긴다.
...
모든 샤드는 동일한 테이블 구조(스키마)를 공유하지만, 각 샤드에 보관되는 데이터 간에는 서로 증분되거나 겹치는 부분이 없다.
...
- **원인:** 하나의 물리적 DB 안에서는 자유롭게 테이블 간 JOIN 연산이 가능했지만, 데이터가 여러 샤드 서버로 물리적으로 찢어지고 나면 여러 샤드에 걸린 데이터를 단 한 번의 쿼리로 조인하눈 것은 사실상 불가능해진다.
...
- **인프라의 모든 계층에 다중화(Replication) 도입**
  - 웹 서버는 로드밸런더 뒷단에 병렬 배정하고, DB는 읽기 전용 사본과 쓰기 전용 원본으로 분리하여 SPOF(단일 장애 지점)을 완전히 제거
...
- **메시지 큐로 서비스 간 결합도를 느슨하게 유지**
  ...
  - Producer와 Consumer의 결합도가 낮아져 한쪽 시스템이 다운되어도 전체 서비스가 마지되지 않는 탄력성을 얻게 됨
- **모니터링 자동화**
