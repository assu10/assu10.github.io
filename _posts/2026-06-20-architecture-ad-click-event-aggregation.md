...
* [4. 상세 설계 2: '정확히 한 번' 전달 보장](#4-상세-설계-2-정확히-한-번-전달-보장)
  ...
  * [4.2. 중복 데이터 처리 아키텍처](#42-중복-데이터-처리-아키텍처)
    ...
    * [4.2.3. 분산 트랜잭션이 필요한 이유](#423-분산-트랜잭션이-필요한-이유)
...
* [6. 결함 내성(Falut Tolerance)와 스냅숏 복구](#6-결함-내성falut-tolerance와-스냅숏-복구)
...
- **Query String:**
  ...
  - window: 분 단위로 표현된 집계 왼도 크기(Integer)
...
여기서는 대규모 분산 쓰기 성능이 강력하게 검증된 **카산드라(Cassandrea)**를 원시 데이터 저장소로 최종 활용한다.
...
- **워크로드 분석**
  ...
  - 반면, 읽기 연산은 백업 및 재계산 시에만 간헐적으로 발생하므로 극단적인 **쓰기 중신(Write-heavy) 시스템**이다.
- **RDBMS의 한계**
  - 전통적인 RDBMS로도 구축 자체는 간가능하겠지만, 초당 50,000건의 고속 쓰기 연산이 지원되는 환경에서는 인덱스(B+ Tree) 갱신 오버헤드와 Lock 경합 때문에 디스크 I/O 병목이 발생한다.
- **대안 아키텍처**
...
- **행 지향(Row-oriented) 방식(RDBMS)**
  ...
  - 특정 유저 한 명의 전체 정보를 조회할 때(`SELECT *`)는 빠르지만, 수십업 건의 데이터 중 '나이의 평균'만 구하고 싶어도 필요없는 이름과 국가 데이터까지 디스크에서 전부 읽어야 하므로 성능이 느려진다.
...
- 이 노드들은 메모리 내부에서 **힙(Heap) 데이터 구조**를 활용하여, 자기가 처리한 데이터 중에서 가장 많이 클릭된 상위 3개의 공고를 효율적으로 식별해 냄
...
<**스타 스키마 접근법의 장점**>
- **직관적인 구조**
  - 모델 자체가 직관적이서 데이터 파이프라인을 설계 및 구축하기 매우 쉬움
- **기존 서비스 재사용 가능**
...
클릭이 발생하지 않은 빈 버킷들까지 데이터베이스 레코드를 차지하게 되어 스토리비 부하가 커질 수 있으므로 꼭 필요한 핵심 차원 위주로 선별 설계해야 한다.
...
| | 장점 | 단점 |
| :--- | :--- | :--- |
| **이벤트 발생 시각** | 광고 클릭 시점을 정확히 알고 있는 클라이언트(디바이스)의 타임스탬프를 쓰므로 **집계 결과가 비즈니스적으로 정확함** | 클클라이언트가 생성한 정보에 의존하므로, 사용자 기기의 시각이 잘못 설정되어 있거나 악성 사용자가 **타임스탬프를 고의로 조작(광고 사기 등)하는 문제**에서 자유로울 수 없음 |
...
처리 시각을 선택했다가 네트워크 장애로 클릭 로그가 몇 시간 뒤에 몰려 들어오면 광조주에게 엉뚱한 시간대의 비용을 청구하게 되는 치명적인 결함이 발생한다.
...
이 문제를 해결하는 워크마크의 구체적인 동작 방식은 아래와 같다.
...
- **고정 윈도(Fixed Window)**
  - 빅데이터 생태계에서 고정 윈도는 **텀블링 윈도와 완전히 같은 개념**으로 통용됨다.
...
분산 환경의 악조건 속에서도 데이터를 정확히 한 번(Exactly-Once) 처리하기 위한 end-to-end 무결성 아키텍처에 대해 알ㄹ아보자.
...
플링크는 데이터 스트림 사이에 체크포인트 배리어(Checkpoint Barrier)라는 특수한 가상 가이드라인을 주입힌다.
...
매년 30%씩 트래픽이 늘어나고 3년바다 규모가 2개가 되는 대용량 환경을 견디려면, 시스템의 전 계층이 독립적으로 Scale-out 할 수 있는 구조를 갖추어야 한다.  
...
- [**소비자(Consumer)의 확장성**](https://assu10.github.io/dev/2026/06/12/architecture-distributed-message-queue-architecture/#495-consumer%EC%9D%98-%ED%99%95%EC%9E%A5%EC%84%B1)
  - 동일한 Consumer Group 내의 Rebalancing 메터니즘을 통해 노드의 추가 및 삭제만으로 시스템 처리 대역폭을 쉽게 늘릴 수 있다.
...
- **파티션 수가 변할 때 생기는 일**
  ...
  - $$10 \pmod 5 = 0$$가 되므로, 동일한 ad001 광고릐 로그이 이제부터 **0번 파티션**으로 들어간다.
...
- **가상 노드가 없는 구조의 문제점**
  - 하나의 물리 서버가 해시 링 위의 단 하나의 거대한 연속 구간 전체를 담당하므로, 서버를 추가하거나 제거할 때 데이터 잽재치 부하가 특정 노드에만 집중되어 클러스터가 쉽게 불안정해진다.
- **가상 노드**
  - 물리 노드 1대가 해시 링 위에 무작위로 파편화된 수많은 작은 토큰(가상 노드 위치)를 나누더 가진다.
  - 예: 노드 1개가 128개의 가상 토큰 보유
- **장점**
  - 클러스터에 새 물리 서버(노드)를 추가하는 순간, 기존의 모든 물리 노드들이 자기가 가진 가상 노드 파편들을 조금씩 떼어내어 새 노드에게 자동으로 나눠준다.
  - 데이터가 클러스터 전체에 완벽하게 균등 분산되므로, 엔지니어가 수동으로 DB 샤딩을 튜닝하거거나 데이터 이관 스크립트를 짤 필요가 없이 Scale-out이 이루어진다.
...
# 6. 결함 내성(Falut Tolerance)와 스냅숏 복구
