...
커맨드는 2가지로 구현할 수 있다.
- 파라메터를 각각 받는 방식
- 커맨드 실행에 필요한 모든 입력값을 포함하는 [파라메터 객체](https://wiki.c2.com/?ParameterObject)로 받는 방식
...
어떤 방식으로 해도 상관없지만 명시적으로 커맨드 구조를 정의하여 다형적으로 전달하는 것을 더 권장한다. (= 파라메터 객체로 받는 방식)
...
> 도메인 이벤트 구독자에게 도메인 이벤트를 안정적으로 게시하는 방법에 대해서는 추후 다룰 예정입니다. (p. 98)
