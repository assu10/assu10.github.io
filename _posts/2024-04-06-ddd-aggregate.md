...
* [2. 애그리거트 루트](#2-애그리거트-루트)
  ...
  * [2.3. 트랜잭션 범위](#23-트랜잭션-범위)
    * [2.3.1. 한 트랜잭션에서 여러 애그리거트 수정](#231-한-트랜잭션에서-여러-애그리거트-수정)
...
* [4. 애그리거트 참조](#4-애그리거트-참조)
  ...
  * [4.2. ID 를 이용한 애그리거트의 간접 참조](#42-id-를-이용한-애그리거트의-간접-참조)
    * [4.2.1. ID 를 이용한 참조와 조회 성능](#421-id-를-이용한-참조와-조회-성능)
...
**도메인 객체 모델이 복잡해지면 개별 구성요소 위주로 모델을 이해하게 되기 때문에 전반적인 구조에서의 도메인 관계를 파악하기 어렵다**.
...
도메인 규칙에 따라 함께 생성되는 구성 요소는 한 애그리거트에 속한 가능성이 높다.  
...
**'A 가 B 를 갖는다' 로 설계할 수 있는 요구사항이 있을 때 A, B 를 한 애그리거트로 묶어서 생각하기 쉬운데 이는 반드시 A 와 B 가 한 애그리거트에 속한다는 것을 의미하지 않는다.**  
...
예) **_Order_ 애그리거트와 관련된 테이블이 3개라면 _Order_ 애그리거트를 저장할 때 애그리거트 루트와 매핑되는 테이블 뿐 아니라 애그리거트에 속한 모든 구성요소에 매핑된 
테이블에 데이터를 저장해야 함**
...
> RDBMS 와 JPA 를 이용하여 리포지터리를 구현하는 방법을 추후 상세히 다룰 예정입니다. (p. 113)
...
'조회 대상이 N 개 일 때 N 개를 읽어오는 한 번의 쿼리와 연관된 데이터를 읽어오는 쿼리를 N 번 실행한다' 해서 이를 `N+1 조회 문제` 라고 한다.
...
> DAO 에 대한 좀 더 상세한 내용은 [9. Spring bean, Java bean, DTO, VO](https://assu10.github.io/dev/2023/05/07/springboot-spring/#9-spring-bean-java-bean-dto-vo) 를 참고하세요.
...
> JPA 를 이용한 모델 매핑과 컬렉션을 사용할 때의 성능 관련 문제는 추후 상세히 다룰 예정입니다. (p. 124)
