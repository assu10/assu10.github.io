...
- `response_type`
  - 클라이언트가 코드를 기대한다는 것을 권한 부여 서버에 알리는 값인 code 를 포함
...
> OAuth 2 암호 그랜트 유형의 예시는 추후 상세히 다룰 예정입니다.

> 리소스 서버가 토큰을 검증하는 방법은 추후 상세히 다룰 예정입니다.
