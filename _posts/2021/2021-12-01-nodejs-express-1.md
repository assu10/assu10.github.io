...
>   - 미들웨어 내용 정리
>   - `multer`
>     - `upload.single('img')` - req.file 객체에 하나의 파일만 업로드
...
비밀키들을 소스 코드에 그대로 적어두면 소스 코드가 유출되었을 때 키도 같이 유출되는데 .env 같은 별도의 파일에 비밀 키를 적어두고
...
### 2.7.1. `upload.single('img')` - req.file 객체에 하나의 파일만 업로드
