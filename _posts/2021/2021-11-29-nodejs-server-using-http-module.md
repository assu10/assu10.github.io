...
res.write, res.end 에 일일이 HTML 을 적는 대신 HTML 을 미리 만들어 두고 그 파일을 fs 모듈로 읽어서 전송하도록 해보자.
...
> GET 메서드의 경우 브라우저에서 캐싱할 수도 있으므로 같은 주소로 GET 요청을 할 때 서버에서 가져오는 것이 아니라 캐시에서 가져올 수도 있다.
...
따라서 보통은 레디스나 맴캐시드 같은 DB 에 저장한다.
