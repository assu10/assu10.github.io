...
let fn = <T>(n: T): T => n
```
하지만 코드가 번거로워지므로 타입스크립트는 타입 변수 부분을 생략할 수 있게 해준다.
...
마지막으로 *mergeArray* 함수를 **순수 함수** 로 구현하려면 매개 변수의 내용이 변경되지 말아야 하기 때문에 매개 변수 타입 앞에 `readonly` 키워드를 넣어준다.
```ts
