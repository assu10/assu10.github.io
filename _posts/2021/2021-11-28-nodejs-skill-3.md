...
fs 는 기본적으로 콜백 형식의 모듈이기 때문에 실무에서 사용하기 불편하다. 따라서 fs 모듈을 프로미스 형식으로 바꿔서 사용하자.
...
이러한 방식은 매우 효율적인 방식으로 많은 수의 I/O 요청이 들어와도 메인 스레드는 백그라운드에 요청 처리를 위임하고 그 후에 요청을 더 받을 수 있다.<br />
...
윈도우는 SET UV_THREADPOOL_SIZE=1, 맥/리눅스는 `UV_THREADPOOL_SIZE=1` 로 **스레드 갯수를 조절**할 수 있다.<br />
