...
이런 서버로 구성된 클러스터의 규모를 CPU 사용률이나 부하, I/O 상태에 따라 자동으로 늘리는 방법은 다양하다.
...
하지만 웹소켓 서버는 Stateful 서버라 기존 서버를 제거할 때는 Gracefully 하게 종료되어야 한다.  
기존 연결부터 종료될 수 있도록 로드밸런서가 인식하는 노드 상태를 '연결 종료 중(draining)'으로 변경해두면 그 서버로는 새로운 웹소켓 연결이 만들어지지 안ㅇㅎ는다.  
그리고 나서 모든 연결이 종료되면 서버를 제거하면 된다.

결국 Stateful 서버 클러스터의 규모를 자동으로 확장하려면 좋은 로드밸런서가 있어야 한다.

---

### 3.1.3. 클라이언트 초기화

모바일 클라이언트는 기동되면 웹소켓 클러스터 내의 서버 가운데 지속성 웹소켓 연결을 맺는다.
'지속성 연결'이라 한 이유는, 연결이 오랜 시간 유지되기 때문이다.

웹소켓 연결이 초기화되면 클라이언트는 해당 모바일 단말의 위치, 즉 해당 단말을 이용 중인 사용자의 위치 정보를 전송한다.
그 정보를 받은 웹소켓 연결 핸들러는 아래 작업을 수행한다.

- 위치 정보 캐시에 보관된 해당 사용자의 위치를 갱신함
- 해당 위치 정보는 뒤이은 계산 과정에 이용되므로 연결 핸들러 내의 변수에 저장해둠
- 사용자 DB를 뒤져 해당 사용자의 모든 친구 정보를 가져옴
- 위치 정보 캐시에 일괄 요청을 보내어 모든 친구의 위치를 한번에 가져온다. 캐시에 보관하는 모든 항목의 TTL은 비활성화 타임아웃 시간과 동일한 값으로 설정되어 있으므로 비활성화 친구의 위치는 캐시에 없다.
- 캐시가 돌려준 친구 위치 각각에 대해 웹소켓 서버는 해당 친구와 사용자 사이의 거리를 게산한다. 그 거리가 검색 반경 이내이면 해당 친구의 상세 정보, 위치, 해당 위치가 마지막으로 확인된 시각은 웹소켓 연결을 통해 클라이언트에 반환한다. (의문점)내 정보,위치,시각이 아니라 해당 친구의 정보를 클라이언트에 주는거야? 왜? 클라이언트에 정보를 주면 클라이언트를 그걸 어디에 사용하는 거지?
- 웹소켓 서버는 각 친구의 레디스 서버버 펍/섭 토픽을 구독한다. 토픽 구독 비용은 저렴하므로 사용자는 친구의 활성화/비활성화 상태에 관계업싱 모든 친구 토픽을 구독할 수 있다.
- 사용자의 현재 위치를 레디스 펍/섭 서버의 전용 토픽을 통해 모든 친구에게 전송한다.

(초기화 때 일어나는 일을 이해하기 쉽게 순서대로 설명해줘.)

---

### 3.1.4. 사용자 DB

사용자 DB엔 두 가지 종류의 데이터가 저장된다.
- 사용자 ID, 이름 등의 사용자 상세 정보
- 친구 관계 데이터

이 두 개의 데이터는 한 대의 RDBMS 서버로는 감당 불가다. 하지만 사용자 ID를 기준으로 샤딩하면 RDBMS라 해도 Scale-out이 가능하다.

주의할 점은 이런 사용자 및 친구 데이터는 DB를 직접 쿼리하는 것이 아니라 내부 API를 통해 이용해야만 한다.  
그렇게 하더라도 시스템 성능에는 큰 차이가 없다.

---

### 3.1.5. 위치 정보 캐시

여기서는 레디스를 사용하였고, 각 항목의 key에는 TTL을 설정하였다.
이 TTL은 해당 사용자의 위치 정보가 갱신될 때마다 초기화되므로 최대 메모리 사용량은 일정 한도 아래로 유지된다.
만일 천만 명의 사용자가 활성화 상태이며 위치 정보 보관에 100byte가 필요하다고 가정하면 최신 레디스 한 대로 모든 위치 정보를 캐시할 수 있다. (의문점) 천만명이 각 100byte면 총 몇 byte야?

하지만 천만 명의 활성 사용자가 대략 30초마다 변경된 위치 정보를 전송한다고 하면 레디스 서버가 감당해야 할 갱신 연산수는 초당 334,000에 달하게 되는데 
이는 최산 고사양 서버를 쓴다고 해도 부담이 된다.
캐시할 데이터는 쉽게 샤딩할 수 있다. 각 사용자의 위치 정보는 서로 독립적인 데이터이므로, 사용자 ID를 기준으로 여러 서버에 샤딩하면 부하 또한 고르게 분배할 수 있다.

---

### 3.1.6. 레디스 펍/섭 서버

여기서는 펍/섭 서버를 모든 온라인 친구에게 보내는 위치 변경 내역 메시지의 라우팅 계층으로 활용한다.
레디스 펍/섭을 선택한 이유는 토픽을 만드는 비용이 아주 저렴하기 때문이다.

구독자가 없는 토픽으로 전송된 메시지는 그대로 버려진다.
토픽 하나를 우지하기 ㅜ이해 구독자 관계를 추적하기 위한 해시 테이블과 연결 리스트가 필요한데 아주 소량의 메모리만을 사용한다.
로프라인 사용자라 어떤 변경도 없는 토픽의 경우 생성된 이후에 CPU 자원을 전혀 사용하지 않는다.

여기서는 그 점을 아래와 같이 활용한다.
- '주변 친구' 기능을 활용하는 모든 사용자에게 토픽 하나씩을 부여한다. 해당 기능을 사용하는 사용자의 앱은 초기화 시에 모든 친구의 채널과 구독 관계를 설정하며 이 때 친구의 상태는 개의치 않는다.
그러면 설계가 단순해진다. 활성화 상태로 바뀐 친구의 토픽을 구독하거나 비활성화가 된 친구의 토픽을 구독 중단하는 등의 작업을 할 필요가 없기 때문이다.
- 유의할 것은 더 많은 메모리를 사용한다는 점인데, 뒤에 살펴보겠지만 메모리가 병목이 될 가능성은 낮다. (뒤의 어디서 나오는지 섹션명 알려줘)
'주변 친구'의 경우 아키텍처를 단순하게 만들 수 있다면 더 많ㅇㄴ 메모리를 투입할 만한 가치가 충분하다.
