...
타입 파라미터는 null 이 될 수 있는 타입을 표시하기 위해 반드시 물음표를 타임 이름 뒤에 붙여야 한다는 규칙의 유일한 예외이다.
...
코트린 표준 라이브러리는 컬렉션을 위한 여러 제네릭 함수를 제공하는데, 제네릭 확장 함수를 사용하려면 수신 객체 앞에 제네릭 명세(괄호로 둘러싼 타입 파라미터 목록)을 위치시키면 된다.
