...
VPC Flow Log 생성 후 데이터를 수집하여 Flow Log Record 를 게시하는데 1분 혹은 10분의 집계 기간에 따라 대기 시간이 소요될 수 있다. (=로그 정보를 실시간으로 게시하지 않음)
...
이제 jhServer-EC2 에서 외부 웹 서버로 HTTP 접근이 정상적으로 되는 것을 확인할 수 있다.
```shell
$ ssh -i "sandbox-jh.pem" ec2-user@43.201.xx.xx
