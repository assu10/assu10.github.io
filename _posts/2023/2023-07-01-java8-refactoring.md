...
이 포스트에서는 람다 표현식을 이용하여 가독성과 유연성을 높이려면 기존 코드를 어떻게 리팩터링해야 하는지 알아본다.  
...
* [1. 가독성과 유연성을 개선하는 리팩터링](#1-가독성과-유연성을-개선하는-리팩터링)
  * [1.1. 익명 클래스를 람다 표현식으로 리팩터링](#11-익명-클래스를-람다-표현식으로-리팩터링)
  * [1.2. 람다 표현식을 메서드 레퍼런스로 리팩터링](#12-람다-표현식을-메서드-레퍼런스로-리팩터링)
  * [1.3. 명령형 데이터 처리를 스트림으로 리팩터링](#13-명령형-데이터-처리를-스트림으로-리팩터링)
  * [1.4. 코드 유연성 개선](#14-코드-유연성-개선)
* [2. 람다로 객체지향 디자인 패턴 리팩터링](#2-람다로-객체지향-디자인-패턴-리팩터링)
...
익명 클래스를 람다 표현식으로 리팩터링하면 간결하고 가독성 좋은 코드를 구현할 수 있다.
