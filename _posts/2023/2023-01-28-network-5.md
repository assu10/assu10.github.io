...
Direct Connect 는 표준 프로토콜인 LACP (Link Aggregation Control Protocol) 을 지원하며, 단일 Direct Connect Location 에서 
다수의 연결을 하나의 논리적인 연결로 구성 가능하다.  
...
AWS Direct Connect 는 BFD 를 지원하여 지정된 임계치 이후에 빠르게 BFD 연결을 끊어 다음 경로로 라우팅할 수 있다.
...
Direct Connect 는 리전 리소스로서 생성된 리전에서만 사용이 가능한데 이 때 Direct Connect Gateway 를 사용하면 다른 리전에서 생성된
...
Transit Gateway 와 Direct Connect Location 사이에 Direct Connect Gateway 가 존재한다.
