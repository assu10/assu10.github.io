...
`UserDetailsService` 와 `PasswordEncoder` 는 자동으로 구성되는 빈이며, 인증 공급자 (`AuthenticationProvider`) 는 이 빈들을 이용하여 사용자를 찾고, 암호를 확인한다.
...
`AuthenticationProvider` 의 기본 구현은 `UserDetailsService`, `PasswordEncoder` 에 제공된 기본 구현을 이용한다.
...
> **NoOpPasswordEncoder**  
> 
> `NoOpPasswordEncoder` 인스턴스는 암호에 암호화나 해시를 적용하지 않고 plain text 처럼 처리하므로 실제 운영 시엔 적절하지 않음
