...
즉, VPC Flow Log 는 정형화된 필드로 이루어진 로그이고 VPC Traffic Mirroring 은 실제 네트워크 패킷이라는 것이다.  
...
최소 요금 및 사전 약정이 없기 때문에 사용한 만큼만 비용을 지불한다.  
...
- `Web ACL` 의 하위 컴포넌트
- 검사 기준을 정의하고 기준을 충족할 경우 수행할 작업(Match Action) 을 포함
...
## 3.4. Resource 삭제
- Web ACL 배포 해제 (*[WAF & Shield] - [AWS WAF] - [Web ACLs] - [생성한 Web ACL 선택] - [Associated AWS resources tab 진입] - [ALB 선택] - [Disassociate]*)
- Web ACL 삭제
