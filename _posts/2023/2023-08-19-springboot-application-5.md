...
- 브라우저가 전송하는 `Accept-Language` 헤더 값을 서버가 인지하여 국제화 기능 제공
  - 사용자 OS 나 브라우저 설정에 따라 사용자가 사용하는 언어 정보가 해당 헤더 값으로 설정되고, 서버 요청 메시지에 자동으로 포함됨
...
`@RestApiController` 가 선언된 ErrorController 에서 발생한 에러는 `@RestControllerAdvice` 가 선언된 클래스인 ApiExceptionHandler 클래스의 
`@ExceptionHandler` 가 적용된 메서드에서 처리한다.

`@RestApiController` 애너테이션이 적용되어 있기 때문에 ApiExceptionHandler 클래스에서 직접 REST-API 로 응답이 가능하다.
...
Java 에서 많이 사용하는 방식은 Slf4J 라이브러리에 로깅 프레임워크를 붙여서 로깅 방식을 구성하는 것이다.  
...
그러면 Java Annotation Preprocessor 가 컴파일 과정에서 Logger 객체를 만들어, 그 객체명은 `log` 가 된다.  
...
스프링 부트 프레임워크의 기본 설정으로는 ch.qos.logback.core.rolling.SizeAndTimeBasedRollingPolicy 클래스를 롤링 정책으로 사용하며,  
...
o.s.beans.factory 패키지 외에서 발생하는 로그 메시지들은 root 설정을 따라서 info 레벨 이상만 로깅이 된다.
```xml
    <logger name="org.springframework.beans.factory" level="warn"/>
...
FileBeat 는 LogStash 와 통합하여 로그를 다시 한 번 정제할 수 있으며, 엘라스틱 서치 검색 엔진에 저장한 후 Kibana 에서 검색하도록 되어있다.  
