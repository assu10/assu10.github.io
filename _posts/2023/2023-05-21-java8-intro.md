...
Java8 에는 Stream API 가 추가되었는데 지금은 일단 단순히 어떤 항목을 연속으로 제공하는 어떠한 기능이라고 생각하면 된다.
...
Stream API 에서는 루프를 신경쓸 필요 없이 라이브러리 내부에서 모든 데이터가 처리된다. (=내부 반복)

또한 Collection 을 이용할 때 많은 요소를 가진 목록을 반복한다면 단일 CPU 로는 힘들수도 있기 때문에 서로 다른 CPU 코어에 작업을 각각 할당해서 처리 시간을 줄이는 것이 좋다.
