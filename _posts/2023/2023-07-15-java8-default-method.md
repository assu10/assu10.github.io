...
* [4. 해석 규칙](#4-해석-규칙)
    * [다른 클래스나 인터페이스로부터 같은 시그니처를 갖는 메서드를 상속받을 때 규칙](#다른-클래스나-인터페이스로부터-같은-시그니처를-같은-메서드를-상속받을-때-규칙)
...
`Comparator.naturalOrder()` 는 표준 알파벳 순서로 요소를 정렬할 수 있도록 Comparator 객체를 반환하는 Comparator 인터페이스에 추가된 static 메서드이다.
...
### 다른 클래스나 인터페이스로부터 같은 시그니처를 갖는 메서드를 상속받을 때 규칙
...
- 클래스가 같은 시그니처를 갖는 여러 디폴트 메서드를 상속하면서 발생하는 충돌 문제를 해결하는 규칙이 있음
