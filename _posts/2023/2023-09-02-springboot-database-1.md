...
스프링 부트 프레임워크는 o.s.boot.autoconfiguration.orm.jpa 패키지에 포함된 HibernateJpaAutoConfiguration 자동 설정 클래스를 제공하며, 
...
데이터베이스와 애플리케이션이 커넥션을 맺기 위해서는 TCP 프로토콜의 3-way-handshake 과정을 거쳐서 TCP 소켓 생성이 되어야 Connection 객체가 생성되는데 
