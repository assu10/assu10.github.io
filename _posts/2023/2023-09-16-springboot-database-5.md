...
Hibernate 프레임워크에서 가장 중요한 역할을 하는 것은 `EntityManager` 클래스이다.
...
`@PersistenceContext` 는 `EntityManager` 를 주입하는 기능만 제공한다.
...
애플리케이션 영역에 포함된 영속성 컨텍스트는 비즈니스 로직과 데이터베이스 사이에 끼어있는 모습이다.
