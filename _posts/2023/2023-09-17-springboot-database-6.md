...
**`@EntityListeners` 애너테이션은 이벤트가 발생하는 엔티티 클래스와 리스너 클래스를 매핑하는 기능**을 제공한다.  
...
JPA/Hibernate 에서는 영속성 컨텍스트의 생명주기를 트랜잭션 범위를 넘어서 스프링 MVC 의 View 까지 확장하여 사용할 수 있는데 이를 **OSIV (Open Session In View)** 라고 한다.  
...
OSIV 는 지연 로딩을 컨트롤러 클래스나 View 에서도 사용할 수 있는 장점이 있지만 아래와 같은 단점이 있다.
...
- 컨트롤러 클래스에서는 다른 엔티티 객체를 참조하는 코드를 사용하지 않아야 함
