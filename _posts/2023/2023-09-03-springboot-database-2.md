...
JPA 가 쿼리를 실행한 결과인 레코드를 엔티티 객체에 매핑하며, JPA 에서는 엔티티 클래스를 정의하는 `@Entity` 애너테이션을 사용한다. 
...
로직으로 기본키를 생성하기 때문에 데이터베이스에 독립적인 방법이다.
...
데이터베이스에서 제공하는 자동 증가 기능으로 기본키를 사용한다.  
...
JSR-310 스펙으로 추가된 자바 클래스들은 JPA 에서 제공하는 속성 변환 기능(AttributeConverter) 기능을 사용하여 변환한다.
