...
> 기본적으로 ForkJoinPool 은 프로세서 개수(= `Runtime.getRuntime().availableProcessors()`) 가 반환하는 값에 상응하는 스레드를 갖는다.  
...
- **각 서브 태스크의 실행 시간은 새로운 태스크를 포킹하는데 드는 시간보다 길어야 함**
  ...
  - 다른 자바 코드처럼 JIT (바이트코드를 기계어 코드로 번역하는 과정) 컴파일러에 의해 최적화되려면 warmed up 또는 실행과정을 거쳐야 하기 때문에 성능 측정 시엔 여러 번 프로그램을 실행한 결과를 측정해야 함
