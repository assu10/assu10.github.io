...
주입 공격은 클라이언트 쪽 스크립트를 주입하는 방식으로 진행된다.
...
위와 같은 상황에서 동일한 리포지터리를 이용하는 다른 기능을 추가하게 된다면 아래와 같은 상황이 벌어질 수 있으므로 
**리포지터리 뿐 아니라 애플리케이션의 모든 계층에 권한 부여를 적용**하는 것이 좋다.

![같은 리포지터리를 사용하는 다른 기능 추가](/assets/img/dev/2023/1111/security_2.png)
...
일체형 웹 애플리케이션은 백엔드와 프론트엔드 간의 직접적인 분리가 없다.
...
토큰의 수명은 고정되어 있으며 일반적으로 오래 유지되지 않는다.

OAuth 2 는 아래와 같은 장점이 있다.

- 클라이언트는 사용자 자격 증명을 저장할 필요 없이 액세스 토큰과 갱신 토큰만 저장하면 됨
- 애플리케이션은 사용자 자격 증명을 노출하지 않음
- 토큰이 탈취되면 사용자 자격 증명을 무효로 할 필요 없이 토큰을 실격시킴
...
API 키를 사용하는 것은 가장 약한 접근법이다. 이 접근법을 이용할 때는 보통 IP 주소 허용 목록을 함께 사용한다.
