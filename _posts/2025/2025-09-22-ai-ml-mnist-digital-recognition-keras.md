...
date: 2025-09-22
categories: ai
tags: ai llm ml deep-learning keras tensorflow mnist python neural-network classification data-preprocessing model-training model-evaluation one-hot-encoding
---
...
**`import matplotlib.pyplot as plt`**
**맷플롯립**은 데이터와 모델의 학습 결과를 그래프로 시각화하여 우리가 직관적으로 이해할 수 있도록 돕는 라이브러리이다.  
...
- 두 번째 Dense 층(은닉층 2): (512개의 입력 * 256개 뉴런) + 256개의 편향 = 131,328개의 파라미터
...
`model.evaluate()` 함수를 사용하면 검증 데이터에 대한 모델의 정확도를 간단하게 확인할 수 있다.
