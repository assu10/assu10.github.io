...
* [2. 도커 기초 명령어](#2-도커-기초-명령어)
  ...
  * [2.6. 컨테이너 내부 접속: `-it`](#26-컨테이너-내부-접속--it)
...
- **도커 호스트(Docker Host)**
  ...
  - 이 데몬은 클라이언트로부터 받은 명령을 해석하고, 이미지 생성, 컨테이너 실행 등 실제 작업을 총괄함
...
아래 그림은 도커 허브에서 Nginx 이미지를 다운로드하는 과정이다.
...
이와 동시에 이 셸 프로세스를 붙잡고 있던 컨테이너도 함께 종료된 것이다.
...
`run`으로 새 컨테이너를 만드는 대신, 이 컨테이너를 다시 시작(`start`)하여 접속(`attach`)할 수 있다.
