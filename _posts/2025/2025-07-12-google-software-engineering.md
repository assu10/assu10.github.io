...
하지만 **하이럼의 법칙이 작용하고, 엔트로피가 낮아지지 않는다고 해서 계획 세우기나 효율성을 포기해서는 안 된다.**  
...
이때는 개발 브랜치 전략을 무한정 브랜치 운영이 아닌 Trunk 기반 개발 방식 등 다른 방식을 고려할 필요가 있다.
...
- **정성적 판단만 가능한 경우**
  ...
  - 이때에도 가능한 지표를 만들고 정량화하려는 노력이 중요함(예: 생산성 스코어, 기술부채 지표 등)
