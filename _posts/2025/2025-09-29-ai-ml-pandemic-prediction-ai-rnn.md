...
* [2. 데이터 가져오기](#2-데이터-가져오기)
* [3. 데이터 정규화 및 분류](#3-데이터-정규화-및-분류)
* [4. 데이터 형태 변경](#4-데이터-형태-변경)
* [5. 입력 데이터 생성](#5-입력-데이터-생성)
* [6. 인공지능 모델에 넣을 형태로 변환](#6-인공지능-모델에-넣을-형태로-변환)
* [7. 인공지능 모델 생성](#7-인공지능-모델-생성)
  * [7.1. 순환 신경망(RNN)의 핵심 원리](#71-순환-신경망rnn의-핵심-원리)
  * [7.2. 모델 아키텍처 구현](#72-모델-아키텍처-구현)
  * [7.3. 모델 구조 요약 분석](#73-모델-구조-요약-분석)
* [8. 모델 학습](#8-모델-학습)
* [9. 데이터 예측 및 결과 역변환](#9-데이터-예측-및-결과-역변환)
* [10. 모델 정확도](#10-모델-정확도)
  * [10.1. 평균 제곱근 오차(RMSE, Root Mean Squared Error)](#101-평균-제곱근-오차rmse-root-mean-squared-error)
* [11. 결과 확인](#11-결과-확인)
...
코로나 19 확진자 수와 같은 시계열(time-series) 데이터를 예측하는 인공지능 모델을 구현하는 방법은 다양하다.  
...
데이터의 순서 자체가 중요한 패턴이므로, 절대로 순서를 섞어서는 안 된다.  
...
다양한 지표가 있지만, 가장 널리 사용되는 지표 중 하나는 [**평균 제곱근 오차(RMSE, Root Mean Squared Error)**](#101-평균-제곱근-오차rmse-root-mean-squared-error)이다.
