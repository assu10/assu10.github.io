...
프로그램은 실행 가능한 명령어들의 정적인 집합으로, 보통 디스크(HDD, SSD) 에 저장되어 있는 상태를 의미한다.  
...
컨테이너가 실행될 때 도커는 새로운 네임스페이스를 생성하고, 컨테이너 프로세스를 그 안에서 실행시켜 **마치 독립된 컴퓨터처럼 동작**하도록 만든다.
