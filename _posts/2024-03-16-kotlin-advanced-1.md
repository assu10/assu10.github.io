...
* [3. `inline`: 람다의 부가 비용 없애기](#3-inline-람다의-부가-비용-없애기)
  * [3.1. 인라이닝이 동작하는 방식](#31-인라이닝이-동작하는-방식)
  * [3.2. 인라인 함수의 한계: `noinline`](#32-인라인-함수의-한계-noinline)
  * [3.3. 컬렉션 연산 인라이닝](#33-컬렉션-연산-인라이닝)
  * [3.4. 함수를 `inline` 으로 선언해야 하는 경우](#34-함수를-inline-으로-선언해야-하는-경우)
  * [3.5. 자원 관리를 위한 `inline` 된 람다 사용: `withLock()`](#35-자원-관리를-위한-inline-된-람다-사용-withlock)
...
> 표준 코틀린 라이브러리가 제공하는 `withLock()` 함수에 대해서는 뒤에 나오는 [3.5. 자원 관리를 위한 `inline` 된 람다 사용: `withLock()`](#35-자원-관리를-위한-inline-된-람다-사용-withlock) 를 참고하세요.
...
## 3.4. 함수를 `inline` 으로 선언해야 하는 경우

---

## 3.5. 자원 관리를 위한 `inline` 된 람다 사용: `withLock()`
