...
★★ 에서는 `Future` 인터페이스가 비동기 계산이 끝났는지 확인할 수 있는 isDone() 메서드, 계산이 끝나길 기다리는 메서드, 결과 회수 메서드 등에 대해 알아볼 것이다.  
...
[4. 비동기 작업 파이프라인](#4-비동기-작업-파이프라인) 에서는 클라이언트가 블록되는 상황을 회피하는 법에 대해 알아본다.  
