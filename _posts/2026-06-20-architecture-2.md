---
layout: post
title:  "Architecture(2) - 광고 클릭 이벤트 집계"
...
기술적인 세부사항을 살펴보기 전에 온라인 광고의 핵심 개념을 먼저 본다.
온라인 광고의 핵심적인 혜택은 실시간 데이터를 통해 광고 효과를 정량적으로 측정할 수 있다는 점이다.

디지털 광고의 핵심 프로세스는 RTB(Real-Time Bidding, 실시간 경매)이다.
이 경매 절차를 통해 광고가 나갈 지면(inventory)를 거래한다.

RTB 프로세스에서 속도는 중요하다. 보통 1초 내에 모든 프로세스가 마무리되어야 하기 때문이다.
(궁금증) 광고 시스템에서 RTB가 무엇인지, 왜 RTB 의 지연 시간은 1초 미만이어야 하는지 설명해줘
데이터 정확성도 중요하다. 
광고 클릭 이벤트 집계는 온라인 광고가 얼마나 효율적이었는지 측정하는데 결정적인 역할을 하며, 결과적으로 광고주가 얼마나 많은 돈을 지불할지에 영향을 미친다.
클릭 집계에 따라 광고 관리자는 광고 예산 조정, 타깃 그룹이나 키워드 변경 등 광고 전략을 수정하기도 한다.

온라인 광고에 사용되는 핵심 지표로는 CTR(Click-Through Rate, 클릭률), CVR(Conversion Rate, 전환률) 등이 있으며, 집계된 광고 클릭 데이터에 기반하여 계산한다.
(궁금증) CVR의 참고 주소는 https://support.google.com/google-ads/answer/2684489?hl=en 이거야. 전환률이라는 것이 정확히 어떤 전환률을 말하는 거야?
...
- 입력 데이터 형태는?
  - 여러 서버에 분산된 로그 파일임
  - 클릭 이벤트는 수집될 때마다 이 로그 파일의 끝에 ㅊ가됨
  - 클릭 이벤트에는 ad_id, click_timestamp, user_id, country 등의 속성이 있음
- 데이터 양은?
  - 매일 10억개의 광고 클릭이 발생하고, 광고는 2백만 회 게재됨
  - 광고 클릭 이벤트 수는 매년 30%씩 증가한다고 가정
- 가장 중요하게 생각해야 할 질의는?
  - 특정 광고에 대한 지난 n분간의 클리게 이벤트 수
  - 지난 1분간 가장 많이 클릭된 광고 100개. 질의 기간과 광고 수는 변경 가능해야 하며, 집계는 매분 이루어짐
  - ip, user_id, country 등의 속성을 기준으로 위 2개 질의 결과를 필터링할 수 있어야 함
- 엣지 케이스에 대해 고려해야 하나? 에상보다 늦게 도착하는 이벤트, 중복된 이벤트, 시스템이 다운된 경우
  - 고려해야 한다.
- 지연 시간 요건은?
  - RTB와 광고 클릭 집계의 지연 시간 요건은 매우 다르다.
  - RTB 지연 시간은 응답성 요구사항 때문에 일반적으로 1초 미만이어야 하지만, 광고 클릭 이벤트 집계는 주로 광고 과금 및 보고에 사용되므로 몇 분 정도의 지연은 허용됨
...
- 데이터 양은 페이스 북이나 구글 규모 (자세한 내용은 1.4 개략적 추정치 참고)

---

## 1.3. 비기능 요구사항

- 집계 결과 정확성은 데이터가 RTB및 광고 과금에 사용되므로 중요
  - (궁금증) 집계 데이터가 RTB에 사용되는 건 아니지 않아?
- 지연되거나 중복된 이벤트 처리 가능해야함
- 부분적인 장애 감내 가능해야 함(견고성, reliability)
- 지연 시간 요구 사항은 전체 처리 시간은 최대 수 분을 넘기지 않아야 함

---

## 1.4. 개략적 추정치

- 일간 능동 사용자(DAU)는 10억명
- 각 사용자는 하루에 평균 1개 광고를 클릭한다고 가정. 따라서 하루에 10억 건의 광고 클릭 이벤트가 발생
- 광고 클릭 QPS = (10^9 이벤트/하루 10^5초 = 10,000)
- 최대 광고 클릭 QPS는 평균 QPS의 5배, 즉 50,000QPS로 가정
- 광고 클릭 이벤트 하나 당 0.1KB 저장 용량이 필요하다고 가정. 따라서 일일 저장소 요구량은 0.1KB * 10억 = 100GB, 월간 저장 요구량은 대량 3TB
...
API 는 클라이언트와 서버 간의 통신 규약이다.
여기서이 클라이언트는 대시보드를 이용하는 데이터 과학자, 광고주 같은 사람들이다.
...
API 1: 지난 n분간 각 ad_id에 발생한 클릭수 집계

- GET /v1/ads/{:ad_id}/aggregated_count (주어진 ad_id에 발생한 이벤트 수를 집계하여 반환)
- query string
  - from: 집계 시작 시간(기본값: 현재 시각부터 1분 전), Long
  - to: 집계 종료 시간(기본값: 혀재 시각), Long
  - filter: 필터링 전략 식별자, 예: filter=001
- Response
  - ad_id: 광고 식별자, String
  - couny: 집계된 클릭 횟수, Long
