...
- [리듀싱: `reduce()`](#1-리듀싱--reduce)
  - [요소의 합](#11-요소의-합)
  - [최대값과 최소값](#12-최대값과-최소값)
- [숫자형 스트림](#2-숫자형-스트림)
  - [기본형 특화 스트림: `IntStream`, `DoubleStream`, `LongStream`](#21-기본형--primitive--특화-스트림--intstream--doublestream--longstream)
  - [숫자 스트림으로 매핑: `mapToInt()`, `mapToDouble()`, `mapToLong()`](#22-숫자-스트림으로-매핑--maptoint--maptodouble--maptolong)
  - [객체 스트림으로 복원: `boxed()`](#23-객체-스트림으로-복원--boxed)
  - [기본값: `OptionalInt`, `OptionalDouble`, `OptionalLong`](#24-기본값--optionalint--optionaldouble--optionallong)
  - [숫자 범위: `range()`, `rangeClosed()`](#25-숫자-범위--range--rangeclosed)
- [스트림 생성](#3-스트림-생성)
  - [값으로 스트림 생성: `Stream.of()`](#31-값으로-스트림-생성--streamof)
  - [배열로 스트림 생성: `Arrays.stream()`](#32-배열로-스트림-생성--arraysstream)
  - [파일로 스트림 생성: `Files.lines()`](#33-파일로-스트림-생성--fileslines)
  - [함수로 무한 스트림 생성](#34-함수로-무한-스트림-생성)
    - [`Stream.iterate()`](#341-streamiterate)
    - [`Stream.generate()`](#342-streamgenerate)
...
> 포크/조인 프레임워크 를 이용하는 방법이 있는데 이는 추후 다룰 예정입니다.
...
> 공급자가 상태를 갖게 되면 생기는 부작용은 추후 자세히 다룰 예정입니다.
