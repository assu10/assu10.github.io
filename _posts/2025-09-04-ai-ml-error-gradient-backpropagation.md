...
**이차 교항 엔트로피이(Binary Cross-Entropy)**다.

이차 교항 엔트로피의 핵심 원리는 아래와 같다.
- 모델이 **정답을 높은 확률로 예측했다면(잘 맞췄다면)** 오차를 **0에 가깝게** 부여
- 모델이 **오답을 높은 확률로 예측했다면(크게 클렸다면)** 오차를 **매우 큰 값으로** 부여
...
그런데 실제 정답이 '20대 이하'라면 오차는 어떻게 계산될까? 이 때 사용하는 손실 함수가 **범주형 교차 엔로피(Categorical Cross-Entropy)**이다.
...
그래프 위의 임의의 지점 b에서 시작한다고 해보자. 목표 지점 a로 이동하려면 어느 방향으로 움ㅁ직여야 할까?  
이 때 활용하는 것이 바로 그 지저에서의 기울기(gradient)이다.
...
요약하면 이 그래프는 **두 점 사이의 평균 기울기(Δy/Δx)**에서 출발하여, 두 저의 간격(Δx)를 극한으로 줄여 **한 점에서의 순간 기울기(미분 계수**)를 구하는 미분의 
기본 원리를 보여준다.
...
- **반복 업데이트**
  - 이전 은닉층은 전달받은 오차 정보를 기반하여 자신의 가중치를 업데이트하고, 다시 그 이전층으로 오차 정보를 전달한다. 이 과정이 바로 다음 층까지 반복된다.
