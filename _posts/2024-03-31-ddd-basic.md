...
이 포스트에서는 도메인 모델, 도메인 모델 패턴, 도메인 모델 도출 과정, 엔티티와 밸류에 대해 알아본다.
...
> 엔티티와 밸류에 대한 추가 설명은 추후 다룰 예정입니다.
